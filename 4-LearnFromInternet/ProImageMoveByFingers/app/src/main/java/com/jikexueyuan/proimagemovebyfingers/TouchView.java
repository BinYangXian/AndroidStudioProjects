...
class TouchView extends ImageView {
    ...
    private int mode = NONE;      //mode用于表示当前处于什么状态
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (MotionEvent.ACTION_MASK & event.getAction()) {  //MotionEvent.ACTION_MASK 表示多点触控事件！！！
            case MotionEvent.ACTION_DOWN:
                mode = DRAG;
                stop_x = (int) event.getRawX();//表示相对于屏幕左上角为原点的坐标
                stop_y = (int) event.getRawY();//同上
                start_x = stop_x - this.getLeft();//用(int) event.getX();一样,表示相对于当前点击Widget（控件）左上角的坐标，这里就是相对于自定义
                                                     // imageView左上角的坐标.建议用前者，如果不是全屏拖动，而是指定范围内，一样适用！
            ...
            case MotionEvent.ACTION_UP:
                ...
                int disX = 0;
                int disY = 0;
                if (getHeight() <= screenH)//如果图片高度小于或等于屏幕高度
                {
                    if (this.getTop() < 0) {//同时图片 顶边 相对于父级容器（及显示框架）的顶边 高度为负数，及超出view显示框上时
                        disY = getTop();     //，记住负偏移量，便于下边回弹
//layout(left , top, right,bottom)函数表示设置view的位置。（其中bottom是 ‘相对于’ 显示框架的 ‘顶边’ 距离像素）
                        this.layout(this.getLeft(), 0, this.getRight(), 0 + this.getHeight());//参数二是回弹位置设置

                    } else if (this.getBottom() >= screenH) {//同时图片底边相对于父级容器（及显示框架）的顶边的高度大于
                                                               // 显示高度，及超出view显示框的底边时
                        disY = getHeight() - screenH + getTop();
                        this.layout(this.getLeft(), screenH - getHeight(), this.getRight(), screenH);//参数二是回弹位置设置
                    }
                } else {
                    int Y1 = getTop();
                ...
                //如果图片缩小到宽高任意一个小于100，那么自动放大，直到大于100.
...
//图片宽度（也就是自定义imageView）必须大于70才可以缩放
    ...
    private void setScale(float temp, int flag) {
        ...
        if (flag == BIGGER) {
//setFrame(left , top, right,bottom)函数表示改变当前view显示所占的框架大小。
