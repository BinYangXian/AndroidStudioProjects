...
public class StationData {
	...
	public void getStationData(double lat, double lon, int distance) {
		Parameters params = new Parameters();//参数在聚合sdk的包里
		params.add("lat", lat);
		params.add("lon", lon);
		params.add("r", distance);
		JuheData.executeWithAPI(7, "http://apis.juhe.cn/oil/region",
				JuheData.GET, params, new DataCallBack() {//callBack由聚合sdk提供

					@Override
					public void resultLoaded(int arg0, String arg1, String arg2) {//参数
					// 1.判断请求是否成功的返回值，2.成功或失败的信息
