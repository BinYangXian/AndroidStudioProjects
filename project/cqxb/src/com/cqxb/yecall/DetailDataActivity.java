...
import com.cqxb.dialog.DialogNativeCade;
...
public class DetailDataActivity extends BaseActivity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//处理 防止null
		if("".equals(phone.getText().toString())||"null".equals(phone.getText().toString())){
			phone.setText("");
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				if("0".equals(online)){//离线
					findViewById(R.id.status).setBackgroundResource(R.drawable.lixian);
					findViewById(R.id.freeCall).setBackgroundResource(R.drawable.callling);
					findViewById(R.id.freeCall).setClickable(true);
				}else {
					findViewById(R.id.status).setBackgroundResource(R.drawable.zaixian);
					findViewById(R.id.freeCall).setBackgroundResource(R.drawable.callling);
					findViewById(R.id.freeCall).setClickable(true);
				}
			...
			case 1:
				...
				findViewById(R.id.status).setBackgroundResource(R.drawable.lixian);
//				findViewById(R.id.freeCall).setBackgroundColor(Color.GRAY);
//				findViewById(R.id.freeCall).setClickable(false);
				if(dialog!=null)dialog.dismiss();
				break;
			case 2:
				findViewById(R.id.freeAccountLayout).setVisibility(View.GONE);
...
//			if(number.length()<10){
//				if("".equals(params)){
//					DialogNativeCade dialog=new DialogNativeCade(DetailDataActivity.this, "确认", "取消");
//					dialog.show();
//					return;
//				}
...
//			@Override
//			public void run() {
//				Intent intent = new Intent(DetailDataActivity.this, InCallActivity.class);
...
//	}
	...
	public void calling(){
		if(SettingInfo.getParams(PreferenceBean.LOGINFLAG, "").equals("")){
			...
			SettingInfo.setParams(PreferenceBean.CALLNAME, name);
			String prefix = "0";
			if(number.indexOf("00")==0 || number.indexOf("013")==0 || number.indexOf("014")==0 || number.indexOf("015")==0 
					|| number.indexOf("016")==0  || number.indexOf("017")==0 || number.indexOf("018")==0 ){
				prefix = "";
			}
	...
	public void callBack(){
		new NetAction().callBack(SettingInfo.getAccount(), number, new OnMyResponseListener() {
			...
			public void onResponse(String json) {
				if (!"".equals(BaseUntil.stringNoNull(json))) {
					...
					if(CommReply.SUCCESS.equals(parseObject.getString("statuscode"))){
						...
						SettingInfo.setParams(PreferenceBean.CALLBACKNAME, name);
//						T.show(getApplicationContext(), String.format(getString(R.string.call_back), number), 1);
					}else {
						T.show(getApplicationContext(), ""+parseObject.getString("reason"), 0);
					}
				}else {
	...
	private BroadcastReceiver broadcastReceiver=new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if("hangUp".equals(intent.getStringExtra("hangUp"))){
				T.show(DetailDataActivity.this, ""+getString(R.string.call_declined), 0);
			}
		}
	};
}
