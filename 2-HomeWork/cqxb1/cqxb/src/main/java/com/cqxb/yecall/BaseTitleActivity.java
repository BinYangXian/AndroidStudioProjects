...
public class BaseTitleActivity extends BaseActivity{
	...
	public View setCustomTextView(int background, int color, int size, String content, View.OnClickListener custom_btn_click_listener) {
		findViewById(R.id.title_custom_bg).setVisibility(View.VISIBLE);
		RelativeLayout layout = (RelativeLayout) findViewById(R.id.title_custom_id);
		layout.setBackgroundResource(background);

		RelativeLayout.LayoutParams rl = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
		rl.addRule(RelativeLayout.CENTER_IN_PARENT);
		TextView customText = new TextView(this);
		customText.setLayoutParams(rl);
		customText.setText(content);
		customText.setTextColor(color);
		customText.setTextSize(size);
		layout.addView(customText);
		if (custom_btn_click_listener != null) {
			findViewById(R.id.title_custom_bg).setOnClickListener(custom_btn_click_listener);
		}
