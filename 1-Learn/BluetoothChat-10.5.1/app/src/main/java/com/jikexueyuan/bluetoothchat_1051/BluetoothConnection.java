...
public class BluetoothConnection{
    ...
    class ManageConnectionThread extends Thread { //AcceptThread侦听成功时，创建第二个辅线程来管理连接
        ...
        public void run() {
            ...
            try {
                out = socket.getOutputStream(); //调用系统的方法获取
                in = socket.getInputStream();   //调用系统的方法获取
                ...
                while ((line = br.readLine()) != null) {
                    if (getOnReadNewLineListener()!=null){ //当调用获得的接口类型的onReadNewLineListener被实现时（既有实例监听者）
                        getOnReadNewLineListener().onRead(line,socket.getRemoteDevice()); //参数2.获取目标设备；1.通过接口通知给外界，回调函数法
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
    ...
    class AcceptThread extends Thread {  //内部类，第一个辅线程
        ...
        public void run() {
            ...
            while (listenning) { //当前serverSocket处于侦听状态的时候，循环接收；否则停掉此。
                ...
                if (socket != null) {
                    cancel();//必须 先 把当前的serverSocket给关闭掉，因为蓝牙的同一个通道（listenUsingRfcommWithServiceRecord）
